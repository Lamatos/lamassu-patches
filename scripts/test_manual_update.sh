...
# Download update.tar file and extract it
wget -qO- "$UPDATE_URL" | tar -x -C /tmp
...
cd /tmp/package || { echo "Failed to change directory to /tmp/package"; ls -la /tmp; exit 1; }
...
cd subpackage || { echo "Failed to change directory to subpackage"; ls -la /tmp/package; exit 1; }
